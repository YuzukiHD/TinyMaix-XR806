...
/******************************* LAYER STRUCT
 * ************************************/
typedef struct tml_head { // 48byte
...
typedef struct tml_gap {
    tml_head_t h;
} tml_gap_t;

typedef struct tml_fc {
    tml_head_t h;
...
typedef struct tml_softmax {
    tml_head_t h;
} tml_softmax_t;

typedef struct tml_reshape {
    tml_head_t h;
} tml_reshape_t;

typedef struct tml_dwconv2d {
    tml_head_t h;
...
#define tm_exp _exp
// maybe some arch have exp acceleration, use macro in arch_xxx.h to
// reload it
#else
#define tm_exp exp
#endif
