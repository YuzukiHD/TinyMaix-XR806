...
#if XRADIO && IS_ENABLED(CONFIG_BT_MESH_SCAN_DISABLE_ASYNC)
void bt_mesh_scan_disable_async(struct k_work *work);
K_WORK_DEFINE(scan_disable_async, bt_mesh_scan_disable_async);
#endif
...
int bt_mesh_scan_disable(void)
{
	...
	/* Fix: we might block while hci rx no buf if scan too much packet and this
	 * function is called in recv packet handling function (called in rx
	 * thread), which means rx thread can't continue running to release rx buf.
	 */
	k_work_submit(&scan_disable_async);
...
#endif /* !(XRADIO && IS_ENABLED(CONFIG_BT_MESH_SCAN_DISABLE_ASYNC)) */
