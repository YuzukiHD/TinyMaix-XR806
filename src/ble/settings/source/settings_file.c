				   ...
				   void *cb_arg, bool filter_duplicates)
{
	...
	vfs = vfs_open(cf->cf_name, VFS_RDWR | VFS_CREAT);
	if (vfs == NULL) {
		return -EINVAL;
	}
	entry_ctx.stor_ctx = vfs;
	entry_ctx.seek = 0;
	entry_ctx.len = 0;
...
static int32_t settings_file_create_or_replace(vfs_file_t **p_vfs,
					   const char *file_name)
{
	struct vfs_info entry;

	if (vfs_stat(file_name, &entry) == 0) {
		if (entry.type == VFS_TYPE_DIR) {
			if (vfs_unlink(file_name)) {
				return -EIO;
			}
		} else {
			return -EISDIR;
	...
	*p_vfs = vfs_open(file_name, VFS_RDWR | VFS_CREAT);
	if (*p_vfs == NULL) {
		return -1;
	}
	return 0;
			   ...
			   size_t val_len)
{
	...
	read_vfs = vfs_open(cf->cf_name, VFS_RDWR | VFS_CREAT);
	if (read_vfs == NULL) {
		return -ENOEXEC;
	}
	...
	if (settings_file_create_or_replace(&write_vfs, tmp_file)) {
		vfs_close(read_vfs);
		return -ENOEXEC;
	}

	loc1.stor_ctx = read_vfs;
	loc1.seek = 0;
	loc1.len = 0;
	loc3.stor_ctx = write_vfs;
	lines = 0;
	...
	rc = vfs_close(write_vfs);
	rc2 = vfs_close(read_vfs);
	rc3 = vfs_unlink(cf->cf_name);

	if (rc == 0 && rc2 == 0 && rc3 == 0) {
		rc3 = vfs_rename(tmp_file, cf->cf_name);
		if (rc3) {
			return -ENOENT;
		}
		cf->cf_lines = lines + 1;
...
static int32_t read_handler(void *ctx, off_t off, char *buf, size_t *len)
{
	...
	vfs_file_t *vfs = entry_ctx->stor_ctx;
	ssize_t r_len;
	int rc;
	...
	rc = vfs_seek(vfs, entry_ctx->seek + off, SEEK_SET);
	if (rc < 0) {
		goto end;
	}

	r_len = vfs_read(vfs, buf, *len);
...
#if 0
	/*we have littlefs and spiffs filesystem, it will goes wrong to create
	dectionry when use spiffs. In order to be consistent in switching between
	the two filesystems, cancel the result check of vfs_mkdir(), add by
	liuzhang, 20210407*/
	rc = vfs_mkdir(CONFIG_SETTINGS_FS_DIR);
...
#else
	vfs_mkdir(CONFIG_SETTINGS_FS_DIR);
#endif
