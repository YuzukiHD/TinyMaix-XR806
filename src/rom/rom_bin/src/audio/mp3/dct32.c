...
#define D32FP(i, s0, s1, s2) { \
    ...
    b0 = a0 + a3;			b3 = AW_MULSHIFT32(*cptr++, a0 - a3) << (s0);	\
	b1 = a1 + a2;			b2 = AW_MULSHIFT32(*cptr++, a1 - a2) << (s1);	\
	buf[i] = b0 + b1;		buf[15-i] = AW_MULSHIFT32(*cptr,   b0 - b1) << (s2); \
	buf[16+i] = b2 + b3;    buf[31-i] = AW_MULSHIFT32(*cptr++, b3 - b2) << (s2); \
}
 ...
 **************************************************************************************/
__sram_text
void AWFDCT32(int *buf, int *dest, int offset, int oddBlock, int gb)
{
	int i, s, tmp, es;
	...
	for (i = 4; i > 0; i--) {
		...
		a4 = buf[4];
		b0 = a0 + a7;
		b7 = AW_MULSHIFT32(*cptr++, a0 - a7) << 1;
		b3 = a3 + a4;
		b4 = AW_MULSHIFT32(*cptr++, a3 - a4) << 3;
		a0 = b0 + b3;
		a3 = AW_MULSHIFT32(*cptr,   b0 - b3) << 1;
		a4 = b4 + b7;
		a7 = AW_MULSHIFT32(*cptr++, b7 - b4) << 1;
		...
		a5 = buf[5];
		b1 = a1 + a6;
		b6 = AW_MULSHIFT32(*cptr++, a1 - a6) << 1;
		b2 = a2 + a5;
		b5 = AW_MULSHIFT32(*cptr++, a2 - a5) << 1;
		a1 = b1 + b2;
		a2 = AW_MULSHIFT32(*cptr,   b1 - b2) << 2;
		a5 = b5 + b6;
		a6 = AW_MULSHIFT32(*cptr++, b6 - b5) << 2;

		b0 = a0 + a1;
		b1 = AW_MULSHIFT32(COS4_0, a0 - a1) << 1;
		b2 = a2 + a3;
		b3 = AW_MULSHIFT32(COS4_0, a3 - a2) << 1;
		buf[0] = b0;
		buf[1] = b1;
		buf[2] = b2 + b3;
		buf[3] = b3;
		...
		b4 = a4 + a5;
		b5 = AW_MULSHIFT32(COS4_0, a4 - a5) << 1;
		b6 = a6 + a7;
		b7 = AW_MULSHIFT32(COS4_0, a7 - a6) << 1;
		b6 += b7;
		buf[4] = b4 + b6;
		buf[5] = b5 + b7;
		buf[6] = b5 + b6;
	...
	d = dest + 64*16 + ((offset - oddBlock) & 7) + (oddBlock ? 0 : AW_MP3_VBUF_LENGTH);
	s = buf[ 0];
	d[0] = d[8] = s;

	/* samples 16 to 31 */
	d = dest + offset + (oddBlock ? AW_MP3_VBUF_LENGTH  : 0);
	...
	d = dest + 16 + ((offset - oddBlock) & 7) + (oddBlock ? 0 : AW_MP3_VBUF_LENGTH);
