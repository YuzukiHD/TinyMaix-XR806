...
static __always_inline unsigned char __tolower(unsigned char c)
{
	if (isupper(c))
		c -= 'A'-'a';
	return c;
...
void print_hex_dump_bytes(const void *addr, size_t len)
{
#ifdef CONFIG_PRINT_HEX_DUMP_WITH_ASSIC
	print_hex_dump(NULL, DUMP_PREFIX_NONE, 16, 1, addr, len, 1);
#else
	unsigned int i;
...
{
#ifdef CONFIG_PRINT_HEX_DUMP_WITH_ASSIC
	print_hex_dump(NULL, DUMP_PREFIX_ADDRESS, 16, 4, addr, len, 1);
#else
	unsigned int i;
