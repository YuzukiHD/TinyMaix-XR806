...
#include "tinymaix/arch_cpu.h"
#elif TM_ARCH == TM_ARCH_ARM_SIMD
#include "tinymaix/arch_arm_simd.h"
#elif TM_ARCH == TM_ARCH_ARM_NEON
#include "tinymaix/arch_arm_neon.h"
#elif TM_ARCH == TM_ARCH_ARM_MVEI
#include "tinymaix/arch_arm_mvei.h"
#elif TM_ARCH == TM_ARCH_RV32P
#include "tinymaix/arch_rv32p.h"
#elif TM_ARCH == TM_ARCH_RV64V
#include "tinymaix/arch_rv64v.h"
#else
...
#if !TM_FASTSCALE
    *outp = (mtype_t)(sumf / out_s + out_zp); //(mtype_t)((int)(sumf/out_s) + out_zp)
    ...
    for (int y = 0; y + CONV_BLK_STEPY <= out->h; y += CONV_BLK_STEPY) {
        ...
        for (int x = 0; x + CONV_BLK_STEPX <= out->w; x += CONV_BLK_STEPX) {
            ...
            if (!slow_flag) { // valid or same valid part
                ...
                for (int c = 0; c < cho; c++) {
                    wtype_t* kptr = (wtype_t*)w + c * maxk;
                    sum0 = sptr[dw_koft[0]] * kptr[0] + sptr[dw_koft[1]] * kptr[1] + sptr[dw_koft[2]] * kptr[2] + sptr[dw_koft[4]] * kptr[3] + sptr[dw_koft[5]] * kptr[4] + sptr[dw_koft[6]] * kptr[5] + sptr[dw_koft[8]] * kptr[6] + sptr[dw_koft[9]] * kptr[7] + sptr[dw_koft[10]] * kptr[8];
                    sum1 = sptr[dw_koft[1]] * kptr[0] + sptr[dw_koft[2]] * kptr[1] + sptr[dw_koft[3]] * kptr[2] + sptr[dw_koft[5]] * kptr[3] + sptr[dw_koft[6]] * kptr[4] + sptr[dw_koft[7]] * kptr[5] + sptr[dw_koft[9]] * kptr[6] + sptr[dw_koft[10]] * kptr[7] + sptr[dw_koft[11]] * kptr[8];
                    sum2 = sptr[dw_koft[4]] * kptr[0] + sptr[dw_koft[5]] * kptr[1] + sptr[dw_koft[6]] * kptr[2] + sptr[dw_koft[8]] * kptr[3] + sptr[dw_koft[9]] * kptr[4] + sptr[dw_koft[10]] * kptr[5] + sptr[dw_koft[12]] * kptr[6] + sptr[dw_koft[13]] * kptr[7] + sptr[dw_koft[14]] * kptr[8];
                    sum3 = sptr[dw_koft[5]] * kptr[0] + sptr[dw_koft[6]] * kptr[1] + sptr[dw_koft[7]] * kptr[2] + sptr[dw_koft[9]] * kptr[3] + sptr[dw_koft[10]] * kptr[4] + sptr[dw_koft[11]] * kptr[5] + sptr[dw_koft[13]] * kptr[6] + sptr[dw_koft[14]] * kptr[7] + sptr[dw_koft[15]] * kptr[8];
...
#error "unsupport mdl type"
                ...
                for (int c = 0; c < out->c; c++) {
                    ...
                    // out_zp);
                    sum0 = sptr[0] * kptr[0] + sptr[1] * kptr[1] + sptr[2] * kptr[2] + sptr[4] * kptr[3] + sptr[5] * kptr[4] + sptr[6] * kptr[5] + sptr[8] * kptr[6] + sptr[9] * kptr[7] + sptr[10] * kptr[8];
                    sum1 = sptr[1] * kptr[0] + sptr[2] * kptr[1] + sptr[3] * kptr[2] + sptr[5] * kptr[3] + sptr[6] * kptr[4] + sptr[7] * kptr[5] + sptr[9] * kptr[6] + sptr[10] * kptr[7] + sptr[11] * kptr[8];
                    sum2 = sptr[4] * kptr[0] + sptr[5] * kptr[1] + sptr[6] * kptr[2] + sptr[8] * kptr[3] + sptr[9] * kptr[4] + sptr[10] * kptr[5] + sptr[12] * kptr[6] + sptr[13] * kptr[7] + sptr[14] * kptr[8];
                    sum3 = sptr[5] * kptr[0] + sptr[6] * kptr[1] + sptr[7] * kptr[2] + sptr[9] * kptr[3] + sptr[10] * kptr[4] + sptr[11] * kptr[5] + sptr[13] * kptr[6] + sptr[14] * kptr[7] + sptr[15] * kptr[8];
                    l_postprocess_sum(sum0, b[c], act, outp + 0 * cho, SUMSCALE, outscale,
                        out_zp);
                    l_postprocess_sum(sum1, b[c], act, outp + 1 * cho, SUMSCALE, outscale,
                        out_zp);
...
#if TM_MDL_TYPE == TM_MDL_INT8 || TM_MDL_TYPE == TM_MDL_INT16
        out->data[c] = (mtype_t)(
            (sum / ((in->h) * (in->w)) - in_zp) * in_s / out_s + out_zp); // requant
#elif TM_MDL_TYPE == TM_MDL_FP32 || TM_MDL_TYPE == TM_MDL_FP16
