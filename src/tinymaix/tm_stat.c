  ...
  for (layer_i = 0; layer_i < b->layer_cnt; layer_i++) {
    ...
    if (h->type < TML_MAXCNT) {
      ...
      case TML_CONV2D: {
        ...
        TM_DBG("Conv2d: kw=%d, kh=%d, sw=%d, sh=%d, dw=%d, dh=%d, act=%d, "
               "pad=[%d,%d,%d,%d], dmul=%d, ws_oft=%d, w_oft=%d, b_oft=%d\n",
               l->kernel_w, l->kernel_h, l->stride_w, l->stride_h,
               l->dilation_w, l->dilation_h, l->act, l->pad[0], l->pad[1],
               l->pad[2], l->pad[3], l->depth_mul, l->ws_oft, l->w_oft,
               l->b_oft);
        break;
      }
      case TML_GAP:
      ...
      case TML_DWCONV2D: {
        ...
        TM_DBG("DWConv2d: kw=%d, kh=%d, sw=%d, sh=%d, dw=%d, dh=%d, act=%d, "
               "pad=[%d,%d,%d,%d], dmul=%d, ws_oft=%d, w_oft=%d, b_oft=%d\n",
               l->kernel_w, l->kernel_h, l->stride_w, l->stride_h,
               l->dilation_w, l->dilation_h, l->act, l->pad[0], l->pad[1],
               l->pad[2], l->pad[3], l->depth_mul, l->ws_oft, l->w_oft,
